...
struct input_dev *inputdev; // 用于输入子系统
...
irqreturn_t  mpu6050_interrupt_thread(int irqno, void *devid)
{
	printk("-------%s-------------\n", __FUNCTION__);
...
int mpu6050_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	inputdev->name = "MPU6050";
	inputdev->id.bustype = BUS_I2C;
	inputdev->dev.parent = &client->dev;

	printk("%d\n",client->irq);
	int ret = request_threaded_irq(client->irq,
				     NULL, mpu6050_interrupt_thread,
				     IRQF_TRIGGER_RISING | IRQF_ONESHOT,
				     "mpu6050", mpu_dev);
	if (ret) {
		dev_err(&mpu_dev->client->dev,
			"can't get IRQ %d, error %d\n", mpu_dev->client->irq, ret);
		goto err_0;
	}
	...
	if (ret) {
		dev_err(&mpu_dev->client->dev, "failed to register input device\n");
		goto err_1;
	}
